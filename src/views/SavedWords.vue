...
export default {
  ...
  data() {
    return {
      loaded: false,
      savedWords: [],
      selectedCsvOptions: ['kana', 'kanji', 'definitions'],
      csvOptions: [
    ...
    updateWords() {
      ...
      Helper.loaded(LoadedEDict => {
        ...
        this.savedWordIds.forEach(id => {
          this.savedWords.push(LoadedEDict.get(id))
        })
      })
    },
    csv() {
      ...
      return this.savedWords
        .map(word => {
          ...
          if (this.selectedCsvOptions.includes('kana')) {
            textToDisplay += `${word.kana}\t`
          }

          if (this.selectedCsvOptions.includes('kanji')) {
            textToDisplay += `${word.kanji}\t`
          }
