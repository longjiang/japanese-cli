    ...
    >
      ...
      >
        <span class="suggestion-not-found">
          <b>&ldquo;{{ text }}&rdquo;</b> is not in <a href="https://github.com/garfieldnate/edict">EDict</a>. Try looking it up in
          <a
  ...
  props: {
    ...
    hrefFunc: {
      ...
      default: function(entry) {
        if (entry) {
          return `#/dictionary/edict/${entry.id}`
        }
      }
    },
    defaultURL: {
    ...
    text() {
      if (this.type === 'dictionary') {
        Helper.loaded(LoadedEDict => {
          this.suggestions = LoadedEDict.lookupFuzzy(this.text, 30)
        })
      }
    }
  },
